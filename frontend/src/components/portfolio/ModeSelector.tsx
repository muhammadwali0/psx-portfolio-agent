...
export default function ModeSelector({ value, onChange }: Props) {
  return (
    <div>
      ...
      <div className="relative grid grid-cols-2 gap-2 p-1 rounded-xl bg-psx-800 border border-psx-500/30 dark:border-psx-500/10">
        {/* Sliding indicator */}
        <motion.div
          layoutId="mode-indicator"
          className="absolute top-1 bottom-1 rounded-lg bg-primary border border-primary/20 shadow-sm"
        ...
        {[
          ...
          <button
            ...
            className={`relative z-10 flex flex-col items-center gap-1 py-3 rounded-lg transition-colors ${value === opt.v ? 'text-selected font-bold' : 'text-psx-300'}`}
          >
